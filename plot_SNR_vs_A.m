function plot_SNR_vs_A(K_prime_minus_L, R, CRC, BG, Modulation, rv_id_sequence, iterations, target_block_errors, target_BLER, EsN0_start, EsN0_delta, seed)
...
% Default values
if nargin == 0
    K_prime_minus_L = (1000:1000:8000) - 24;
    R = 1/3;
    CRC = 'CRC24B';
    BG = 1;
    Modulation = 'QPSK';
...
for R_index = 1:length(R)
    ...
    set(plots(R_index),'XData',K_prime_minus_L+24);
    legend(cellstr(num2str(R(1:R_index)', 'R=%0.2f')),'Location','eastoutside');
    
    EsN0s = nan(1,length(K_prime_minus_L));
    ...
    for K_prime_minus_L_index = 1:length(K_prime_minus_L)
        ...
        try
            ...
            EsN0 = EsN0_start;
            prev_EsN0 = nan;
            E = round((K_prime_minus_L(K_prime_minus_L_index)+24)/R(R_index)/hMod.Q_m)*hMod.Q_m
            
 
                hEnc = NRLDPCEncoder('BG',BG,'CRC',CRC,'K_prime_minus_L',K_prime_minus_L(K_prime_minus_L_index),'E',E,'Q_m',hMod.Q_m)
                hDec = NRLDPCDecoder('BG',BG,'CRC',CRC,'K_prime_minus_L',K_prime_minus_L(K_prime_minus_L_index),'E',E,'Q_m',hMod.Q_m,'I_HARQ',1,'iterations',iterations);
            ...
            if strcmp(ME.identifier, 'polar_3gpp_matlab:UnsupportedBlockLength')
                warning('polar_3gpp_matlab:UnsupportedBlockLength','%s does not support the combination of block lengths A=%d and E=%d. %s',code,A(K_prime_minus_L_index),E(R_index), getReport(ME, 'basic', 'hyperlinks', 'on' ));
                continue
            else
                rethrow(ME);
            end
        ...
        EsN0s(K_prime_minus_L_index) = interp1(log10([prev_BLER, BLER]),[prev_EsN0,EsN0],log10(target_BLER));
        ...
        fprintf(fid,'%d\t%f\n',K_prime_minus_L(K_prime_minus_L_index),EsN0s(K_prime_minus_L_index));
