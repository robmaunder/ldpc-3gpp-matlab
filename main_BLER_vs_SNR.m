function main_BLER_vs_SNR(BG, K_prime, E, iterations, target_block_errors, target_BLER, EsN0_start, EsN0_delta, seed)
...
for BG_index = 1:length(BG)
    ...
    title(['3GPP New Radio LDPC code, BG = ',num2str(BG(BG_index)),', E = ',num2str(E(E_index)),', iterations = ',num2str(iterations),', errors = ',num2str(target_block_errors),', QPSK, AWGN']);
    ylabel('BLER');
    xlabel('E_s/N_0 [dB]');
    ylim([target_BLER,1]);
    hold on
    ...
    for K_prime_index = 1:length(K_prime)
        ...
        filename = ['results/BLER_vs_SNR_',num2str(BG(BG_index)),'_',num2str(E(E_index)),'_',num2str(K_prime(K_prime_index)),'_',num2str(iterations),'_',num2str(target_block_errors),'_',num2str(seed)];
        ...
        try
            ...
            hEnc = NRLDPCEncoder('BG',BG(BG_index),'K_prime',K_prime(K_prime_index),'E',E(E_index))            
            hDec = NRLDPCDecoder2('BG',BG(BG_index),'K_prime',K_prime(K_prime_index),'E',E(E_index),'iterations',iterations);
            ...
            while BLER > target_BLER
                ...
                while keep_going && block_error_counts(end) < target_block_errors
                    ...
                    b = round(rand(K_prime(K_prime_index),1));                    
